...
We will see a quick setup to configure the ``cryptpad`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``cryptpad`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``cryptpad`` formulas under ``/srv/salt/cryptpad/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/cryptpad/parameters/dns:domain/
    ln -s dns:domain /srv/salt/cryptpad/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/cryptpad/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/cryptpad/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``cryptpad`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``cryptpad`` formulas under ``/srv/salt/cryptpad/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/cryptpad/parameters/roles
...
We create a configuration for the role ``cryptpad/server`` in ``/srv/salt/cryptpad/parameters/roles/cryptpad/server.yaml``:
...
We create another configuration for the role ``cryptpad/client`` in ``/srv/salt/cryptpad/parameters/roles/cryptpad/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles cryptpad/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles cryptpad/client
...
.. code-block:: console
    ...
    mv /srv/salt/cryptpad/parameters/dns:domain/  '/srv/salt/cryptpad/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-cryptpad-as-json:
      test.nop:
        - name: {{ cryptpad | json }}
...
Here is an example based on `template-formula/cryptpad/config/file.sls`_:
    ...
    cryptpad-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='cryptpad-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ cryptpad.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``cryptpad`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ cryptpad['winner'] }}
...
.. _template-formula/cryptpad/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/cryptpad/config/file.sls
